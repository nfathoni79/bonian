    ...
    public function create($name, $options = [])
    {
        $default = [
            'templates' => [
                ...
                'nest' => '<div class="m-menu__submenu"><span class="m-menu__arrow"></span><ul{{attrs}}>{{items}}</ul></div>',
                'item' => '<li{{attrs}}>{{link}}{{nest}}</li>',
                'link' => '<a href="{{url}}"{{attrs}}><i class="{{icon}}">{{span}}</i><span class="m-menu__link-text">{{label}}</span>{{arrow}}</a>',
                'text' => '<span{{attrs}}>{{label}}</span>'
            ],
            'menuAttributes' => [
                'class' => 'm-menu__nav  m-menu__nav--dropdown-submenu-arrow'
            ],
            'ancestorClass' => 'm-menu__item--open m-menu__item--expanded',
            'branchClass' => 'm-menu__item--submenu',
            'currentClass' => 'm-menu__item--active'
        ];
        if ($options !== []) {
            $default += $options;
        }
        ...
        foreach($menu as $key => $val) {
            ...
            if ($item) {
                $item->addChild($val['name'], [
                        ...
                        'templates' => [
                            'text' => '<span{{attrs}}><span class="m-menu__link-text"> {{label}}</span></span>'
                        ]
                    ]
                );
                $_menu = $item->addChild($val['name'], array_replace_recursive($hasChild ? $this->_hasChildOption : $this->_childOption, [
                    'uri' => $val['url'],
                    'templateVars' => [
                        'icon' => $val['icon']
                    ]
