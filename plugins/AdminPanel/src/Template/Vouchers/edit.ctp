...
<script>
    $( document ).ready(function() {
        ...
        $('.btn-pick').on('click',function (){
            ...
            var idcategori = $(this).data('idcategori');

            $.get("<?= $this->Url->build(['action' => 'getCategoryPath' ]); ?>/"+idcategori, function(data, status){
                $('#level1').selectpicker('val', data[0].id);
                setChainCategory('#level2',  data[0].id, function() {
                    $('#level2').selectpicker('val', data[1].id);
                });
            });

        })

        $('.voucher-delete').on('click',function (){
            var deleterow = $(this).data('id');

            $.post( "<?= $this->Url->build(['action' => 'deleteVoucherDetails']); ?>/" + deleterow, { _csrfToken: '<?= $this->request->getParam('_csrfToken'); ?>' } )
                .done(function( data ) {
        ...
        function setChainCategory(target, parent_id, cb) {
            $.ajax({
                type: 'POST',
