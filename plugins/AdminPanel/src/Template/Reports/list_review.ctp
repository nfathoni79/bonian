        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__body">
                ...
                <div class="m_datatable">
                    ...
                    <table class="table table-striped- table-bordered table-hover table-checkable" id="table-list-review">
                        <thead>
                        <tr>
                            <th>ID</th>
                            <th>Nama Customer</th>
...
<script>
    ...
    function delete_data(id) {
        $.post( "<?= $this->Url->build(['action' => 'deleteReview']); ?>/" + id, { _csrfToken: '<?= $this->request->getParam('_csrfToken'); ?>' } )
            .done(function( data ) {
                location.href = '<?= $this->Url->build();?>';
            });
    }

    var datatable = $('#table-list-review').DataTable({
        ...
        order: [[0, 'desc']],
        ajax: {
            url: "<?= $this->Url->build(['action' => 'listReview', $this->request->params['pass'][0]]); ?>",
            type: 'POST',
            data: {
                pagination: {
                    perpage: 50,
