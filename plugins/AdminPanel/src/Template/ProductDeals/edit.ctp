            ...
            <div class="m-portlet__body">
                ...
                <div class="form-group m-form__group m--margin-top-10">
                    <div class="alert m-alert m-alert--default" role="alert">
                        Produk yang akan dijual dalam flash sale <a href="javascript:void(0);" class="btn btn-success m-btn m-btn--custom m-btn--icon add-flash-sale"><span><i class="la la-plus-circle"></i> Tambah Produk Flash Sale</span></a>
                    </div>
                </div>
                <div class="row mt-3 p-flash-sale">
                    <?php foreach($ProductDealDetail as $k => $vals):?>
                    <input type="hidden" name="ProductDealDetails[<?php echo $k;?>][idx]" class="form-control form-control-danger m-input m-input--air" value="<?php echo $vals['id'];?>">
                    <div class="col-md-12 rowing-<?php echo $k;?>">
                        <div class="form-group m-form__group row">
                            <label class="col-form-label col-lg-2">Nama Produk</label>
                            <div class="col-lg-4">
                                <div class="m-typeahead">
                                    <input type="text" name="ProductDealDetails[<?php echo $k;?>][produk]" placeholder="Pencarian Produk" class="form-control form-control-danger m-input m-input--air k_typeahead" dir="ltr" data-row="<?php echo $k;?>" value="<?php echo $vals['product']['name'];?>">
                                    <a href="javascript:void(0);" class="btn btn-danger m-btn m-btn--custom m-btn--icon delete-flash-sale mt-3" data-row="<?php echo $k;?>" data-key="<?php echo $vals['id'];?>"><span><i class="la la-trash"></i> Hapus dari daftar produk</span></a>
                                </div>
                            </div>
                            <div class="col-lg-5 offset-1 info-detail-<?php echo $k;?>">
                                <div class="m-widget5 pull-left product-lists" data-product-id="<?= $vals['product']['id']; ?>">
                                    <div class="m-widget5__item">
                                        <div class="m-widget5__content">
                                            ...
                                            <div class="m-widget5__section">
                                                ...
                                                <input type="hidden" name="ProductDealDetails[<?php echo $k;?>][produk_id]" class="form-control form-control-danger m-input m-input--air" value="<?php echo $vals['product']['id'];?>">
                                                <input type="text" name="ProductDealDetails[<?php echo $k;?>][price_sale]" class="form-control form-control-danger m-input m-input--air" placeholder="Harga Flash Sale" value="<?php echo $vals['price_sale'];?>">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
?>
<script>
    $('.delete-flash-sale').click(function (event) {
        if (confirm('Are you sure you want to delete this?')) {
            $.ajax({
                url: '<?= $this->Url->build(['action' => 'deleteDetail']); ?>',
                type: "POST",
                data: {
                    id: $(this).data('key'),
