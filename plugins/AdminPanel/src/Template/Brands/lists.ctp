                ...
                <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        <li class="m-portlet__nav-item">
                            <a href="<?= $this->Url->build(['action' => 'addList']); ?>" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
                                <span>
...
<script>
    ...
    function delete_data(id) {
        $.post( "<?= $this->Url->build(['action' => 'deleteList']); ?>/" + id, { _csrfToken: '<?= $this->request->getParam('_csrfToken'); ?>' } )
            .done(function( data ) {
            ...
            var datatable = $('#table-brands').mDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            method: 'POST',
                            url: '<?= $this->Url->build(['action' => 'lists']); ?>',
                            cache: false,
                            params: {
                ...
                columns: [
...

                    {
                        ...
                        title: 'Logo',
                        template: function(row) {
                            return row.logo ? '<img src="<?= $this->Url->build('/files/Brands/logo/'); ?>'+row.logo+'" style="width: 100px;">' : '-';
                        }
                    },
                    ...
                    /** Action button **/
                    {
                        ...
                        overflow: 'visible',
                        template: function (row, index, datatable) {
                            return '<a href="javascript:view_data('+row.id+');" class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="VIew"><i class="la la-eye"></i></a><a href="<?= $this->Url->build(['action' => 'editList']); ?>/'+ row.id +'"class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="Edit"><i class="la la-edit"></i></a><a href="javascript:delete_data('+row.id+');" onclick="return confirm(\'Are you sure delete #'+row.id+'\');" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Delete"><i class="la la-trash"></i></a>';
                        }
                    }
                ]
            });
